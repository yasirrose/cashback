<template>
  <b-row>
    <b-col cols="12">
      <b-card-code>
        <b-link :to="{ path: 'add-store' }" class="custom-btn">Add Store</b-link>
        ...
        >
          <template slot="table-row" slot-scope="props">
            ...
            <!-- Column: image -->
            <span  v-else-if="props.column.field === 'image'" class="text-nowrap">
              <b-img
                :src="`/image/${props.row.image}`"
                rounded
                height="80"
              />
            </span>
            ...
            <span v-else-if="props.column.field === 'featured'">
              <b-badge
                ...
                :variant="statusVariant(props.row.featured)"
              >Not Featured</b-badge>
              <b-badge
                v-else-if="props.row.featured === 1"
                :variant="statusVariant(props.row.featured)"
              >Featured</b-badge>
            </span>
            ...
            <span v-else-if="props.column.field === 'action'">
              <span>
                  <b-link :to="{ path: `update-store/${props.row.id}` }" title="Edit Product"><feather-icon icon="Edit2Icon"/></b-link>
  ...
  methods: {
    ...
    confirmation(data, type) {
      this.$swal({
        ...
        if (result.value) {
          if (type == "batchDelete") {
            this.deleteMultiStores(data);
          } else if (type == "singleDelete") {
            this.deleteStore(data);
          }
        }
      });
    },
    ...
    deleteMultiStores(data) {
      Admin.deleteMultiStores(
        data,
        resp => {
          if (resp.success) {
            this.$toast({
